...
<%@ page isELIgnored="false" %>
 ...
 <link  href="<c:url value="/static/resources/css/jquery-ui.css" />" rel="stylesheet">
 ...
 <script src="<c:url value="/static/resources/js/jquery.validate.min.js" />"></script>
  ...
  <script src="<c:url value="/static/resources/js/jquery-1.10.2.js" />"></script>
  <script src="<c:url value="/static/resources/js/jquery-ui.js" />"></script>
...
$("#registerVehicle").click(function(){
	window.location.href="/VehicleManagement/RegisterVehicle";
});
...
$("#Entries").click(function(){
	...
	{    $("#entrymsg").html("Field Should not be empty!");
 	 ...
 	 $("#entrymsg").hide();
	 $.ajax(
			 {url: "/VehicleManagement/AdminViewEntries?data="+$("#datepicker").val()+"&dateTime"+new Date(), success: function(result){
...
$("#violations").click(function(){
	...
	{    $("#viomsg").html("Field Should not be empty!");
	 ...
	 $.ajax({url: "/VehicleManagement/AdminViolationDetails?userid="+data+"&dateTime"+new Date(), success: function(result){
          var data = result.substring(0,result.length-1);
		  var x = data.split("#") ;      
		  var y;
...
$("#sendEmails1").click(function(){
	...
	$.ajax({url: "/VehicleManagement/sendMailStart?dateTime"+new Date(), success: function(result){
...
$("#sendEmails2").click(function(){
	...
	$.ajax({url: "/VehicleManagement/sendMailEnd?dateTime"+new Date(), success: function(result){
...
$("#viewStatus").click(function(){
	...
	$.ajax({url: "/VehicleManagement/adminViewStatus?userid="+data+"&dateTime"+new Date(), success: function(result){
        ...
        var data = result;
  		...
  		$("#updateUserStat").click(function(){
			 var statusdata = $("#stat").val()+"@@"+$("#editStatus").val();
		    	 ...
		    	 $.ajax({url: "/VehicleManagement/AdminUpdateStatus?data="+statusdata+"&dateTime"+new Date(), success: function(result){
		    	    if(result == "Success")
					{
...
<div style="float:right">Status : ${status}  &nbsp;&nbsp; <a href="/VehicleManagement/logout">Logout</a></div>
	...
	<br/><br/>
		...
		<legend align="right">Vehicles by Date in Rented </legend>
			<br/>
		...
		<br/>
		<fieldset>
		<legend align="left">Register New Vehicle</legend>
			<br/>
			<input type="button" id="registerVehicle" value="Register Vehicle"/>
			<br/><br/>
		</fieldset>
		<br/>
		<fieldset>
